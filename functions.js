function disableControlsHeader(target, conatinerIdentifier) {
	if (target.tagName === 'DIV' || target.id === 'header') {
		...
		if (!newScheduleOptions.hidden) {
			...
			document
				.getElementById('header-inputs-1')
				.setAttribute('hidden', 'hidden');
			document
				.getElementById('header-inputs-2')
				.setAttribute('hidden', 'hidden');
			document
		...
		if (!importExportOptions.hidden && conatinerIdentifier !== '2') {
			const buttonImportSchedule = document.getElementById(
				'import-button-1'
			);
			...
			const buttonImportSchedule2 = document.getElementById(
				'import-button-2'
			);
			buttonImportSchedule2.removeAttribute('style');
			buttonImportSchedule2.isactive = 'false';
			const buttonExportSchedule = document.getElementById(
				'export-button'
			);
			...
			document
				.getElementById('import-selected-container')
				.setAttribute('hidden', 'hidden');
			document
				.getElementById('import-container')
				.setAttribute('hidden', 'hidden');
			document
				.getElementById('export-container')
				.setAttribute('hidden', 'hidden');
			document
...
function receivedText(e) {
	...
	try {
		...
		alert(
			'El archivo seleccionado no parece contener agendas de esta aplicación'
		);
		return;
	}
			...
			importedSchedules.forEach(importedSchedule => {
				if (typeof importedSchedule !== 'object') {
					...
					for (let i = 1; i !== titles.length; i++) {
						saveData(
							`${importedSchedule}-${i}`,
							JSON.parse(imported[`${importedSchedule}-${i}`])
						);
					}
				}
			});
...
function renderBlocks(name) {
	...
	if (titlesLength > 1) {
		...
		for (let i = 1; i !== titlesLength; i++) {
			...
			blocksSample.length > maxLength
				? (maxLength = blocksSample.length)
				: maxLength;
		}
...
function check(element) {
	const [identifier, isTitle, description, input, type] = elementToModify(
		element
	);
...
function isCheckAll(identifier, isTitle) {
	...
	counterChecks === counterDescriptions
		? (checkAll.innerHTML = '✓')
		: (checkAll.innerHTML = '');
...
function checkAll(element) {
	...
	for (let i = 0; i < liLength; i++) {
		...
		const descriptionToStyle = document.getElementById(
			`description-${checkElement.id.split('-')[1]}`
		);
		if (description.innerHTML) counter++;
		if (description.innerHTML && !checkElement.innerHTML) {
			const [blockArray, blockNumber, indexArray] = blockId(
				checkElement.id.split('-')[1]
			);
...
function blockSave(identifier, isTitle, textToInsert, type, indexToFill) {
	const [blockArray, blockNumber, indexArray, name] = blockId(
		identifier,
		isTitle,
		type
	);
	...
	if (isTitle === 't') {
		...
		if (titlesLength > 1) {
			type === 'check'
				? (blockArray[indexArray][0] = textToInsert)
...
function deleteAllSchedules() {
	...
	const buttonsContainer2 = document.getElementById(
		'header-schedule-options-2'
	);
	buttonsContainer2.setAttribute('hidden', 'hidden');
	const buttonsContainer3 = document.getElementById(
		'header-schedule-options-3'
	);
	buttonsContainer3.setAttribute('hidden', 'hidden');
	const buttonsContainer4 = document.getElementById(
...
function toggleBlockOptions(identifier) {
	...
	const hideOptionsIconsContainer = document.getElementById(
		`hideOptionsIconsContainer-${identifier}`
	);
...
function disableBlockOptionsManagement(identifier) {
	const type = blockOptionsContainerActive[identifier];
	if (!type) return;
	const hideManageContainer = document.getElementById(
		`${type}HideManageContainer-${identifier}`
	);
	...
	if (type === 'copy') {
		...
		const buttonContainer = document.getElementById(
			`${type}ButtonContainer-${identifier}`
		);
	...
	if (type === 'clean') {
		...
		const cleanButtonContainer = document.getElementById(
			`${type}ButtonContainer-${identifier}`
		);
		const sortButtonContainer = document.getElementById(
			`sortButtonContainer-${identifier}`
		);
...
function blockSelector(type, title, i, n) {
	const selector = elementsCreator(
		'select',
		`${type}Selector-${i + 1}`,
		`${type}Selector`,
		false,
		{ name: `${type}Block`, title, actual: i + 1 }
	);
...
function copyBlock(type, identifier) {
	...
	showBlockOptionsContainers(
		...
		identifier,
		'copy',
		'Se reemplazará el contenido de este bloque'
	);
...
function confirmActionBlock(type, identifier) {
	const hideManageContainer = document.getElementById(
		`confirmHideManageContainer-${identifier}`
	);
			...
			if (titles.length > 2) {
				...
				for (let i = Number(identifier); i < titles.length; i++) {
					...
					if (isTitleFormat.length === 2) {
						if (
							isTitleFormat[0] === 'Título' &&
							Number(isTitleFormat[1]) === i + 1
						) {
...
function touchScreen(isTouchScreen) {
	const buttonsOptionsContainer2 = document.getElementById(
		'header-schedule-options-2'
	);
	const buttonsOptionsContainer3 = document.getElementById(
		'header-schedule-options-3'
	);
	const buttonsOptionsContainer4 = document.getElementById(
		'header-schedule-options-4'
	);
	const toggleTouchscreenButton = document.getElementById(
		'toggle-touchscreen'
	);
	const blockAdvancedOptions = document.querySelectorAll(
