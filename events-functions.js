function newSchedule(element) {
	element.addEventListener('click', () => {
		if (
			!(
				element.id === 'import-button-1' ||
				element.id === 'import-button-2' ||
				element.id === 'export-button'
			)
		) {
			document
				.getElementById('options-header-1')
		...
		const buttonImportSchedule1 = document.getElementById(
			'import-button-1'
		);
		...
		if (element.id === 'new-schedule') {
			...
			document
				.getElementById('options-header-2')
			...
			if (
				...
				document
					.getElementById('header-schedule-options-2')
					.setAttribute('hidden', 'hidden');
				document
					.getElementById('header-schedule-options-3')
					.setAttribute('hidden', 'hidden');
				document
					.getElementById('header-schedule-options-4')
					.setAttribute('hidden', 'hidden');
			...
			if (element.id === 'import-button-1') {
				...
				document
					...
					.setAttribute('hidden', 'hidden');
				document
					.getElementById('import-export-buttons')
					.removeAttribute('hidden');
				document
...
function addSchedule(element) {
	element.addEventListener('click', () => {
		...
		const buttonsImportExport = document.getElementById(
		...
		if (userButtons.hidden) {
			if (
				!buttonsOptionsContainer2.hidden ||
				!buttonsImportExport.hidden
			) {
...
function create(element) {
	element.addEventListener('click', () => {
		...
		const inputDescriptionsNumber = document.getElementById(
			'descriptions-number'
		);
		...
		if (allowInitialData) blockConstructor(true, allowInitialData);
			...
			blockConstructor(
				...
				false,
				[{ name: scheduleName }],
				inputBlocksNumber.value,
				inputDescriptionsNumber.value
			);
...
function importExportButtons(element) {
	element.addEventListener('click', () => {
		...
		const optionsHeader2 = document.getElementById('options-header-2');
		const buttonExportSchedule = document.getElementById('export-button');
		const importExportContainer = document.getElementById(
...
function deleteSchedule(element) {
	element.addEventListener('click', () => {
		...
		if (schedules.length === 2) {
			const isConfirmed = confirm(
				'Borrar esta agenda es una acción que no se puede deshacer'
			);
			if (isConfirmed) {
				deleteAllSchedules();
			} else return;
		} else {
			const deleteThisSchedule = confirm(
				'Borrar esta agenda es una acción que no se puede deshacer'
			);
			if (deleteThisSchedule) {
				...
				const indexToDelete = schedules.findIndex(
