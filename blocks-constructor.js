...
function blockConstructor(
	...
	allowInitialData
		? (m = dataToInsert[0].descriptionsInBlock)
		: (m = descriptionsNumber);
	const blocksContainer = document.getElementById('blocks-container');
	const scheduleBlocks = [];
	isNew && scheduleBlocks.push(name);
	for (let i = 0; i < n; i++) {
		// level 1
		const block = elementsCreator('DIV', `block-${i + 1}`, 'block', null, {
			counter: 'hidden'
		});
		...
		const deleteBlock = elementsCreator(
			'img',
			`deleteBlock-${i + 1}`,
			'icon-button-block',
			null,
		...
		const [moveSelector, moveArray] = blockSelector(
			'move',
			'Mover bloque',
			i,
			n
		...
		const [copySelector, copyArray] = blockSelector(
			'copy',
			'Copiar bloque',
			i,
			n
		...
		const copyButton = elementsCreator(
			'span',
			`copyButton-${i + 1}`,
			'button-darkgreen',
			'Copiar'
		...
		severalAppends(copyManageContainer, [
			labelCopyBlock,
			copySelector,
			copyButtonContainer
		]);
		...
		severalAppends(cleanManageContainer, [
			cleanText,
			cleanButtonContainer,
			sortButtonContainer
		]);
		...
		const confirmText = elementsCreator(
			'span',
			`confirmText-${i + 1}`,
			'confirmText'
		);
		...
		severalAppends(blockAdvancedOptions, [
			blockAdvancedPrimaryContainer,
			optionsBlockButton
		]);
		const title = elementsCreator('DIV', `title-${i + 1}`, 'title');
		const titleNumber = elementsCreator(
			'SPAN',
			`titleNumber-${i + 1}`,
			'titleNumber',
			`${i + 1}`
		...
		let headingText = '';
		allowInitialData
			? (headingText = dataToInsert[i + 1][0])
			: (headingText = `Título ${i + 1}`);
		...
		const blockCounter = elementsCreator(
			'DIV',
			`block-counter-${i + 1}`,
			'block-counter'
		);
		...
		const checkAll = elementsCreator(
			'DIV',
			`checkall-${i + 1}`,
			'checkall',
			null,
		...
		const checksCounterContainer = elementsCreator(
			'div',
			`checkcounter-${i + 1}`,
			'checkcounter'
		);
		const span1 = elementsCreator('span', `span1-${i + 1}`, 'span1');
		const span2 = elementsCreator(
			'span',
			`span2-${i + 1}`,
			'span2',
			' de '
		...
		const emptySpace = elementsCreator(
			'div',
			`emptyspace-${i + 1}`,
			'emptyspace'
		);
		const percentage = elementsCreator(
			'div',
			`percentage-${i + 1}`,
			'percentage'
		);
		...
		if (counterChecks === counterDescriptions && counterDescriptions)
			checkAll.innerHTML = '✓';
		list.appendChild(ol);
		severalAppends(block, [
			blockAdvancedOptions,
			title,
			blockCounter,
			list
		]);
		blockEventDetector(block);
		counterDescriptionsChecks(
			ol,
			false,
			span1,
			span3,
